...
function smartTruncate(text, maxChars = 7000) {
  if (text.length <= maxChars) return text;
  // Keep first 60% + last 20% — intro + conclusion most important
...
- 意译为主，根据中文语境重新表达，不逐字翻译
- 技术术语保留英文或加括号注明（如 LLM、RAG、fine-tuning）
- 每篇给出：中文标题、2-3句核心摘要、完整翻译正文（段落间空行）

文章列表：
${list}

请严格按JSON格式返回，不要输出其他内容：
{"results":[{"index":0,"titleZh":"中文标题","summary":"摘要","content":"正文翻译"},{"index":1,...},...]}`;
}
...
export async function* translateArticleStream(url, title, fallbackDesc, apiOpts) {
  ...
  if (isShort) {
    ...
    try {
      ...
      const parsed = parseStructured(raw);
      cache.set(url, { ok: true, ...parsed, url });
