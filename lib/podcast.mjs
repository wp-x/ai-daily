...
async function runPodcastPipeline(task) {
  ...
  try {
    // Step 1: Check CLI + copy cookie to default path (download audio bug workaround)
    update('running', 5, '检查 notebooklm-py...');
    // notebooklm download audio ignores --storage flag, always reads default path
    try {
      const { mkdirSync, copyFileSync } = await import('fs');
      const { homedir } = await import('os');
      const defaultDir = homedir() + '/.notebooklm';
      mkdirSync(defaultDir, { recursive: true });
      copyFileSync(COOKIE_FILE, defaultDir + '/storage_state.json');
      console.log('[podcast] Cookie synced to default path');
    } catch(e) { console.warn('[podcast] Cookie sync failed:', e.message); }
    ...
    if (added === 0) throw new Error('所有文章添加失败，请检查链接是否有效');
        ...
        // Step 4a: 启动 generate audio（不加 --wait，立即返回 task ID）
    update('running', 55, '开始生成播客音频（约 10-20 分钟）...');
    ...
    const genRes = await runNLM([
      ...
      '--language', 'zh_Hans',
      '--length', 'long',
      customInstructions,       // 无 --wait，立即返回
    ], 60 * 1000);              // 只等1分钟启动

    console.log(`[podcast] generate audio 启动 ok:${genRes.ok} out:${(genRes.stdout||'').slice(0,300)} err:${(genRes.stderr||'').slice(0,200)}`);
    if (!genRes.ok) throw new Error(`启动生成失败: ${genRes.error || genRes.stderr}`);

    // Step 4b: 解析 artifact task ID，用 artifact wait 轮询（最长20分钟）
    const artifactId = (genRes.stdout + genRes.stderr)
      .match(/Task:\s*([a-f0-9-]{36})/)?.[1];
    console.log(`[podcast] artifact ID: ${artifactId}`);
    update('running', 60, `音频生成中，等待 NotebookLM 完成…（artifact: ${artifactId?.slice(0,8) || '解析中'}）`);

    // 轮询 artifact list 等待 completed（最长25分钟）
    const pollStart = Date.now();
    const POLL_MAX = 25 * 60 * 1000;
    const POLL_INTERVAL = 30 * 1000;
    let artifactFullId = null;
    let pollCount = 0;
    while (Date.now() - pollStart < POLL_MAX) {
      await new Promise(r => setTimeout(r, POLL_INTERVAL));
      pollCount++;
      const listRes = await runNLM(['artifact', 'list', '--notebook', notebookId, '--json'], 30000);
      if (listRes.ok) {
        try {
          const listData = JSON.parse(listRes.stdout);
          const audioArtifact = listData.artifacts?.find(a => a.type_id === 'audio');
          if (audioArtifact) {
            artifactFullId = audioArtifact.id;
            console.log(`[podcast] poll#${pollCount} artifact=${audioArtifact.id.slice(0,8)} status=${audioArtifact.status}`);
            update('running', 60 + Math.min(20, pollCount), `音频生成中… (${Math.floor((Date.now()-pollStart)/60000)}分钟)`);
            if (audioArtifact.status === 'completed') break;
            if (audioArtifact.status === 'failed') throw new Error('NotebookLM 音频生成失败');
          }
        } catch(e) { console.warn('[podcast] parse artifact list failed:', e.message); }
      }
    }
    if (!artifactFullId) throw new Error('超时：25分钟内未生成音频');
    console.log(`[podcast] 音频生成完成，artifact: ${artifactFullId}`);

    // Step 5: Download MP3
    update('running', 85, '下载 MP3...');
    console.log(`[podcast] 开始下载 MP3`);
    const filename = `podcast-${task.taskId}.mp3`;
    const mp3Path = join(PODCAST_DIR, filename);

    const dlArgs = ['download', 'audio', '--notebook', notebookId, '--force', mp3Path];
    if (artifactFullId) dlArgs.splice(3, 0, '--artifact', artifactFullId);
    else dlArgs.splice(3, 0, '--latest');
    const dlRes = await runNLM(dlArgs, 5 * 60 * 1000);

    console.log(`[podcast] download 完成，ok: ${dlRes.ok}, out: ${(dlRes.stdout||'').slice(0,200)}, err: ${(dlRes.stderr||'').slice(0,200)}`);
    ...
    console.log(`[podcast] �� 完成！文件: ${filename}`);
