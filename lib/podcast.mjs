...
async function runPodcastPipeline(task) {
  ...
  try {
    ...
    if (artifactId) {
      const waitRes = await runNLM([
        'artifact', 'wait', artifactId,
        '--notebook', notebookId,
        '--timeout', '1200',    // 最长20分钟
      ], 22 * 60 * 1000);
      console.log(`[podcast] artifact wait ok:${waitRes.ok} out:${(waitRes.stdout||'').slice(0,200)} err:${(waitRes.stderr||'').slice(0,200)}`);
      if (!waitRes.ok) throw new Error(`等待音频生成失败: ${waitRes.stderr || waitRes.error}`);
    } else {
      console.log('[podcast] 未解析到 artifact ID，等待15分钟后直接下载...');
